...
def rData(tn, seed, N0, N1): 
    ...
    x5 = np.hstack((s5,b5))
    x6 = np.hstack((s6,b6))
    # Stack target values horizontally and convert to integers
    ...
    df['Type'] = pd.Categorical(targets, tn)
    # Add target values
    df['Targets'] = targets
...
# ---------------------------------------------------------------------------------
'''
# Open .csv file as data frame
df = pd.read_csv('/home/rmr/Sentinelhs/Spain/spanishTargets2.csv')

# Convert column to list
NAT = df['Nationality'][:].tolist()

# Add colum
df['Dis_Nat'] = dn_mad + dn_bar + dn_val
